...
if (!class_exists('BLAZE_DEMO_IMPORTER_Demo_Importer')) {
    ...
    final class BLAZE_DEMO_IMPORTER_Demo_Importer {
        ...
        public static function generate_plugin_label($plugin) {
            ...
            switch ($status) {
                case 'install' :
                    $btn_label = esc_html__('Install', 'blazethemes-demo-importer');
                    break;

                case 'inactive' :
                    $btn_label = esc_html__('Activate', 'blazethemes-demo-importer');
                    break;

                case 'active' :
                    $btn_label = esc_html__('Installed', 'blazethemes-demo-importer');
                    break;
            }
            return $btn_label;
        }
        ...
        public function plugin_installer_callback() {
            ...
            if (!current_user_can('install_plugins')) {
                wp_send_json(
                        array(
                            'success' => false,
                            'message' => esc_html__('Sorry, you are not allowed to install plugins on this site.', 'blazethemes-demo-importer')
                        )
                );
                die();
            }
            ...
            if (!wp_verify_nonce($nonce, 'plugin_installer_nonce')) {
                wp_send_json(
                        array(
                            'success' => false,
                            'message' => esc_html__('Error - unable to verify nonce, please try again.', 'blazethemes-demo-importer')
                        )
                );
                die();
            }
        ...
        public function plugin_offline_installer_callback() {
            ...
            if ($file_location) {
                ...
                wp_send_json(
                        array(
                            'success' => false,
                            'message' => __('Missing File Location.', 'blazethemes-demo-importer')
                        )
                );
            }
        ...
        public function plugin_activation_callback() {
            ...
            if (file_exists($plugin_file)) {
                ...
                wp_send_json(
                        array(
                            'success' => false,
                            'message' => __('Error!! Plugin not activated.', 'blazethemes-demo-importer')
                        )
                );
            }
        ...
        public function get_local_dir_path($file_location, $plugin) {
            ...
            if (isset($file_location)) {
                ...
                $url = wp_nonce_url(admin_url('themes.php?page=blazethemes-demo-importer-recommended-plugins'), 'remote-file-installation');
